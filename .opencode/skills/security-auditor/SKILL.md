...
When auditing inside this repository, load `unsorted-architecture` first so
auth (JWT admin/customer split), the RLS policy model, and the secrets/config
flow (`backend/config/env.js`) are evaluated against the real design.